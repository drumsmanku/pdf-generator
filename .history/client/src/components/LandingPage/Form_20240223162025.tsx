...
const Form = ({ onFormSubmit }: { onFormSubmit: (data: any) => void }) => {
  ...
  const submitForm = async () => {
    if (validateFields(2)) {
      try {
        const response = await fetch('https://fix-health-tm4f.onrender.com/submit-form', {
          method: 'POST',
          headers: {
        ...
        if (response.ok) {
          ...
          // Now reset the form fields and slide index
          setFormData({
            name: '',
            phoneNumber: '',
            age: '',
          ...
          setSlideIndex(0); // Set back to the first slide
